...
function Assert-T1OSVirtualBoxConfiguration {
    ...
    $info = @(& $VBoxManage showvminfo $Name --machinereadable 2>$null)
    if ($LASTEXITCODE -ne 0) {
        throw "the VirtualBox VM '$Name' is not registered. Run scripts/build vbox.ps1."
    }
    ...
    if ($problems.Count -gt 0) {
        throw "the VirtualBox VM configuration is incomplete or points at the wrong media: $($problems -join '; '). Run scripts/build vbox.ps1."
    }
}
...
Assert-T1OSArtifactCurrent `
    ...
    -RebuildCommand 'scripts/build vbox.ps1'
Assert-T1OSArtifactCurrent `
