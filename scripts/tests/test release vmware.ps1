...
$evidencePath = Join-Path $softwareRoot 'vmware-release-smoke-serial.log'
$token = [guid]::NewGuid().ToString('N').Substring(0, 10)
$cloneName = "The One OS 0.31 Release Smoke $token"
$cloneRoot = Join-Path $softwareRoot "vmware-smoke-$token"
...
if (-not (Test-Path -LiteralPath $sourceVmx -PathType Leaf)) {
    throw "VMware VM not found: $sourceVmx. Run scripts/vm/build vmware.ps1."
}
...
try {
    ...
    if (Test-Path -LiteralPath $cloneRoot) {
        ...
        if (-not $resolvedCloneRoot.StartsWith($resolvedSoftwareRoot, [System.StringComparison]::OrdinalIgnoreCase)) {
