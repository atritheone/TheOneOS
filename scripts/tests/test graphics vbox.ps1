...
if ($Matrix) {
    ...
    $fourKOutput = @(& pwsh -NoLogo -NoProfile -NonInteractive -File (Join-Path $PSScriptRoot '..\test.ps1') -GraphicsCompositor 2>&1)
...
function Set-CpuBootOverride {
    ...
    $createIso = Join-Path $PSScriptRoot '..\vm\create iso.ps1'
...
try {
    ...
    if (-not $vmExists) {
        throw "VirtualBox VM '$VmName' is not registered. Run scripts/vm/build vbox.ps1 first."
    }
