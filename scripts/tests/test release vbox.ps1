...
$evidencePath = Join-Path $softwareRoot 'vbox-release-smoke-serial.log'
$token = [guid]::NewGuid().ToString('N').Substring(0, 10)
$cloneName = "$VmName 0.31 Release Smoke $token"
$cloneBase = Join-Path $softwareRoot "vbox-smoke-$token"
$registered = $false
$bootAccepted = $false
...
try {
    ...
    if (Test-Path -LiteralPath $cloneBase) {
        ...
        if (-not $resolvedCloneBase.StartsWith($resolvedSoftwareRoot, [System.StringComparison]::OrdinalIgnoreCase)) {
