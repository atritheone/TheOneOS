...
$initScript = Join-Path $projectRoot 'resource\entry\init\init hardware.sh'
$angelContract = Join-Path $projectRoot 'resource\entry\init\ANGEL.md'
...
$ntfsCheckerProductPatch = Join-Path $projectRoot 'resource\entry\roothealth\0001-roothealth-read-only-checker.patch'
$ntfsCheckerSecurityPatch = Join-Path $projectRoot 'resource\entry\roothealth\0002-ntfs-3g-2026.7.7-index-hardening.patch'
$ntfsCheckerVerdictPatch = Join-Path $projectRoot 'resource\entry\roothealth\0003-roothealth-verdict-hardening.patch'
...
$grubScript = Join-Path $projectRoot 'resource\entry\grub\grub hardware 0.2.cfg'
$encryptedGrubScript = Join-Path $projectRoot 'resource\entry\grub\grub hardware encrypted 0.2.cfg'
$grubTheme = Join-Path $projectRoot 'resource\entry\grub\t1os hardware theme.txt'
$grubBackground = Join-Path $projectRoot 'resource\entry\grub\t1os black background.png.base64'
$kernelConfig = Join-Path $projectRoot 'resource\entry\kernel\T10Skernel hardware 0.19 settings.txt'
$kernelPolicy = Join-Path $projectRoot 'resource\entry\kernel\t1os_lsm.c'
...
$chromiumProviderSource = Join-Path $projectRoot 'resource\entry\chromium\t1os_path_provider.c'
$chromiumInputBridge = Join-Path $projectRoot 'source\software\chromium\tools\t1os-xinput'
$chromiumInputBridgeSource = Join-Path $projectRoot 'resource\entry\chromium\t1os_xinput.c'
$chromiumWindowManager = Join-Path $projectRoot 'source\software\chromium\tools\t1os-xwm'
$chromiumWindowManagerSource = Join-Path $projectRoot 'resource\entry\chromium\t1os_xwm.c'
...
$chromiumSubprocessSource = Join-Path $projectRoot 'resource\entry\chromium\t1os_chrome_subprocess.c'
