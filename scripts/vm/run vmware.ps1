...
Assert-T1OSArtifactCurrent `
    ...
    -RebuildCommand 'scripts/vm/build vmware.ps1'
Assert-T1OSArtifactCurrent `
...
foreach ($requiredSetting in @(
    ...
    if (-not $vmxText.Contains($requiredSetting)) {
        throw "the VMware configuration is incomplete or points at the wrong media. Missing: $requiredSetting. Run scripts/vm/build vmware.ps1."
    }
}
...
if ($qemuImg) {
    ...
    if ([int64]$diskInfo.'virtual-size' -ne $rawLength) {
        throw "the VMware disk has the wrong virtual size ($($diskInfo.'virtual-size') instead of $rawLength). Run scripts/vm/build vmware.ps1."
    }
}
