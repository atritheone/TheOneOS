...
$pythonIndex = Join-Path $softwareRoot 't1os-python-index'
$profiledPythonConfig = Join-Path $projectRoot 'source\python\build\runtime.json'
$manifestPath = Join-Path $softwareRoot 't1os-vm-test-base.json'
$templateRoot = Join-Path $softwareRoot 'vm-test-template'
...
function Remove-T1OSTemplateDirectory {
    ...
    $allowed = [System.IO.Path]::GetFullPath($softwareRoot).TrimEnd('\') + '\'
...
New-Item -ItemType Directory -Path $softwareRoot -Force | Out-Null
