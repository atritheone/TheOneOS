...
struct _ntfs_volume {
	...
	u64 max_flb_cnt;
	/*
	 * Full-occupancy oracle for the fsck allocator barrier.  The pass-1 MFT
	 * scan sets every cluster referenced by a valid inode here, and
	 ...
	 * Set-only during the scan, so it shares the FB_ONES/NULL sentinel
	 * scheme but needs neither clear nor duplicate detection.
	 */
