...
struct ntfs_device {
	...
	/*
	 * RootHealth's product build never lets libntfs reach a raw device
	 * pwrite backend.  Its installed callback records a typed in-memory
	 * operation and makes later reads observe the staged image.  The callback
	 * is NULL for ordinary libntfs users; this fork's roothealth-only
	 * ntfs_pwrite() then fails closed rather than bypassing the typed writer.
	 */
	s64 (*d_roothealth_plan_write)(struct ntfs_device *dev,
			const void *buf, s64 count, s64 offset, void *opaque);
	void *d_roothealth_plan_opaque;
};
...
extern int ntfs_device_roothealth_install_plan_write(struct ntfs_device *dev,
		s64 (*callback)(struct ntfs_device *dev, const void *buf,
			s64 count, s64 offset, void *opaque), void *opaque);
extern void ntfs_device_roothealth_remove_plan_write(struct ntfs_device *dev);
