		...
		LCN start_lcn, const NTFS_CLUSTER_ALLOCATION_ZONES zone)
{
	...
	while (1) {
		...
		/*
		 * The on-disk $Bitmap is untrustworthy during fsck: a cluster
		 * can be in use yet unmarked, and handing it out would create a
		 * duplication.  Fold in the fsck occupancy oracle (built by the
		 * pass-1 MFT scan) so the search sees the union of on-disk and
		 * known occupancy and never allocates a cluster an inode owns.
		 */
		if (NVolFsck(vol))
