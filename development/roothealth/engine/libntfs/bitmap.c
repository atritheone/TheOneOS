...
/*
 * Once fsck builds its in-memory mft bitmap, that copy is authoritative:
 * the final bitmap apply overwrites the disk with it.  Any record the
 * library allocates or frees in between only flips the on-disk bit, so
 * mirror those flips into the fsck bitmap or the apply would revert them.
 */
...
unsigned long _find_first_zero_bit(const unsigned long *addr, unsigned long size)
{
	...
	for (idx = 0; idx * BITS_PER_LONG < size; idx++) {
		if (addr[idx] != ~0UL)
			return idx * BITS_PER_LONG + ffzl(addr[idx]) - 1;
	}
 ...
 */
static inline
unsigned long ntfs_find_first_zero_bit(const unsigned long *addr, unsigned long size)
{
	if (size > 0 && size <= BITS_PER_LONG) {
		unsigned long val = *addr | ~GENMASK(size - 1, 0);

		return val == ~0UL ? size : ffzl(val) - 1;
	}
