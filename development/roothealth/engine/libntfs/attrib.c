		...
		ntfschar *name, u32 name_len)
{
	...
	if (na->type == AT_DATA && na->name == AT_UNNAMED &&
		...
		/*
		 * The stream is the authority on its own state; the
		 * $STANDARD_INFORMATION flags are only a cached copy.  Under
		 * fsck resync the inode flags instead of refusing the open:
		 * the refusal would end with the whole file discarded over a
		 * repairable one-bit mismatch.
		 */
		if (NVolFsck(ni->vol)) {
		...
		BOOL is_fsck, ATTR_RECORD *a, u64 inum, BOOL *fixed)
{
	...
	/*
	 * Attribute checks for $MFT/$MFTMirr run with the volume's fsck
	 * state cleared, which suppresses every prompt; keep repairing
	 * those two silently as part of bringing the volume up.  All other
	 * records go through the regular problem flow, so no-repair mode
	 * neither modifies the record nor lets the caller write it back.
	 */
	if (NVolFsck(vol)) {
		pctx.inum = inum;
		pctx.a = a;
	...
	if (!ret) {
		...
		switch(a->type) {
			case AT_FILE_NAME :
				...
				/*
				 * Every $FILE_NAME is a key in the parent's $I30
				 * index, so its resident_flags must carry
				 * RESIDENT_ATTR_IS_INDEXED.  FILE_ATTR_NOT_CONTENT_INDEXED
				 * is unrelated -- it only excludes the file's contents
				 * from the Windows search index -- so it must not gate
				 * this repair, or the flag never gets fixed on the many
				 * files that set it.
				 */
			...
			case AT_INDEX_ROOT :
				...
				/*
				 * Under fsck every INDEX_ROOT header fault is
				 * repaired field by field later on (see
				 * ntfsck_repair_index_root_fields()); rejecting
				 * the record here would instead discard the
				 * whole directory over one fixable header byte.
				 * Refuse only what that repair cannot work
				 * with: a non-resident attribute or a value too
				 * short to hold even an empty index.
				 */
				if (is_fsck) {
					if (a->non_resident
							|| (le32_to_cpu(a->value_length)
