		...
		struct ntfs_device_operations *dops, void *priv_data)
{
	...
	if (dev) {
		...
		dev->d_roothealth_plan_write = NULL;
		dev->d_roothealth_plan_opaque = NULL;
	}
	return dev;
}

int ntfs_device_roothealth_install_plan_write(struct ntfs_device *dev,
		s64 (*callback)(struct ntfs_device *dev, const void *buf,
			s64 count, s64 offset, void *opaque), void *opaque)
{
	if (!dev || !callback || dev->d_roothealth_plan_write) {
		errno = EINVAL;
		return -1;
	}
	dev->d_roothealth_plan_write = callback;
	dev->d_roothealth_plan_opaque = opaque;
	return 0;
}

void ntfs_device_roothealth_remove_plan_write(struct ntfs_device *dev)
{
	if (!dev)
		return;
	dev->d_roothealth_plan_write = NULL;
		...
		const void *b)
{
	...
	if (!count)
		return 0;
	if (!dev || !dev->d_roothealth_plan_write) {
		errno = EPERM;
		goto out;
	}
	...
	for (total = 0; count; count -= written, total += written) {
		written = dev->d_roothealth_plan_write(dev,
				(const char*)b + total, count, pos + total,
				dev->d_roothealth_plan_opaque);
		/* If everything ok, continue. */
		if (written > 0)
			continue;
		/*
