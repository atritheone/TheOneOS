		...
		INDEX_BLOCK *ib, u32 block_size, u64 inum, VCN vcn)
{
	 ...
	 * a repair.  ntfsck_repair_index_block() restores the magic (with the
	 * other header fields) when the index is validated.
	 */
	return (0);
}
		...
		ntfs_index_context *ictx)
{
	...
	if (ie->ie_flags & INDEX_ENTRY_NODE) {
		 ...
		 * follows the aligned key directly, but view indexes ($SDH/$SII
		 * in $Secure, $O/$Q in $Quota, $O in $ObjId, $R in $Reparse)
		 * carry a data part between the key and the VCN, so the entry is
	...
	/*
	 * Only a filename index entry ends right after its aligned key, so
	 * only there does "exactly eight bytes past the key" prove a missing
	 * sub-node VCN.  A view index entry carries a data part between the
	 * key and the VCN, and a small payload lands on the same length --
	 * $Quota/$O maps a 16-byte SID key to a 4-byte owner id -- so the
	...
	/*
	 * An end entry carries no key, so the overflow checks below do not
	 * apply.  Return @ret rather than 0: the repairs above may have turned
	 * this very entry into an end entry, and callers only write the entry
	 * back when a positive value says something changed.
	 */
...
int ntfs_index_lookup(const void *key, const int key_len, ntfs_index_context *icx)
{
	 ...
	 * ntfs_ie_lookup() takes the end of the node from index_length, so a
	 * corrupt length would send the walk past the MFT record buffer.  A
	 * record with such a header is accepted at inode open under fsck (the
	 * header is repaired later by the index checks); fail the lookup
	 * cleanly until then.
	 */
		...
		INDEX_ENTRY *ie, INDEX_BLOCK *ib)
{
	...
	if (le16_to_cpu(ie_dup->key_length) > 0) {
		...
		ntfs_index_ctx_reinit(icx);
		dummy = ntfs_index_lookup(&ie_dup->key, le16_to_cpu(ie_dup->key_length), icx);
	}
out:
	free(ie_roam);
	if (ie_dup)
...
static int ntfs_index_rm_leaf(ntfs_index_context *icx)
{
	...
	while (ntfs_ie_get_vcn(ie) != icx->parent_vcn[icx->pindex + 1]) {
	...
	do {
		...
		if (!ntfs_ib_read(ictx, vcn, ictx->ib)) {
			...
			ret = ntfs_index_entry_inconsistent(ictx->ni->vol, entry,
					ictx->ir->collation_rule, ictx->ni->mft_no,
					ictx);
			if (ret > 0) {
...
INDEX_ENTRY *ntfs_index_next(INDEX_ENTRY *ie, ntfs_index_context *ictx)
{
	...
	if (ie->ie_flags & INDEX_ENTRY_END)
		...
		ret = ntfs_index_entry_inconsistent(vol, next, cr,
				ictx->ni->mft_no, ictx);
		if (ret > 0) {
			ret = ntfsck_update_index_entry(ictx);
			if (ret) {
