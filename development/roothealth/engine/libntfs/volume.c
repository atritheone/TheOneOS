...
/*
 * Canonical NTFS 3.x $AttrDef table, byte-for-byte the layout mkntfs writes
 * (mirror of attrdef_ntfs3x_array in src/attrdef.c - keep the two in sync).
 * Used to regenerate a $AttrDef that fails ntfs_attrdef_check().
 */
static const unsigned char ntfs_attrdef_default[2560] = {
 ...
 */
static BOOL ntfs_attrdef_check(const ntfs_volume *vol)
{
	const ATTR_DEF *ad;
	const u8 *base = (const u8 *)vol->attrdef;
	s32 len = vol->attrdef_len;
	u32 prev_type = 0;
	BOOL have_si = FALSE, have_fn = FALSE, have_data = FALSE;

	if (!base || len < (s32)sizeof(ATTR_DEF))
		return FALSE;

	for (ad = vol->attrdef;
			(((const u8 *)ad - base) + (s32)sizeof(ATTR_DEF) <= len)
				&& ad->type;
			++ad) {
		...
		if (type == le32_to_cpu(AT_STANDARD_INFORMATION))
			have_si = TRUE;
		else if (type == le32_to_cpu(AT_FILE_NAME))
			have_fn = TRUE;
		else if (type == le32_to_cpu(AT_DATA))
			have_data = TRUE;
	}

	return have_si && have_fn && have_data;
}
...
static int ntfs_attrdef_repair(ntfs_volume *vol)
{
	...
	ntfs_attr *na = NULL;
	void *buf;
	s64 sz = (s64)sizeof(ntfs_attrdef_default);
	int ret = -1;
	...
	if (!na) {
		/*
		 * $DATA is gone entirely.  Recreating it means allocating
		 * clusters, but ntfs_fsck_mount() does not set up the fsck
		 * cluster bitmaps until after ntfs_mount() returns, so a
		 * cluster allocation here would dereference them while still
		 * NULL.  Leave this as a mount-time limitation and fail without
		 * touching the in-core copy so the caller aborts the mount.
		 */
		ntfs_inode_close(ni);
		return -1;
	}
	...
	if (!buf)
		goto out;
	memcpy(buf, ntfs_attrdef_default, sz);
	free(vol->attrdef);
...
ntfs_volume *ntfs_device_mount(struct ntfs_device *dev, ntfs_mount_flags flags)
{
	...
	for (i = 0; (i < vol->mftmirr_size) && (i < FILE_first_user); ++i) {
		...
		if (!ntfs_mft_record_check(vol, FILE_MFT + i, mrec)) {
			...
			/*
			 * $MFT and $MFTMirr disagree.  $MFT is structurally
			 * valid, but the default action - overwriting the
			 * mirror with $MFT - is only safe if $MFT is also
			 * semantically valid.  Semantic barrier: for a named
			 * system file, if $MFT is semantically dead while the
			 * mirror copy is both structurally and semantically
			 * sound, the mirror is the better copy - recover $MFT
			 * from it instead of destroying the last good copy.
			 */
			if (i < 12 &&
...
upcase_ok:
	...
	/*
	 * Read the attribute-definition table.  A missing, oversized or short
	 * $DATA leaves vol->attrdef NULL; the validation step below then treats
	 * that exactly like corrupt content and regenerates it in fsck mode,
	 ...
	 * mis-drive every attribute bound/resident check the fsck passes make,
	 * so in fsck mode regenerate the default table.  If the fix is declined
	 * but a table was loaded, keep it and carry on (the previous behaviour -
	 * mount never validated $AttrDef content); if nothing usable remains the
	 * mount cannot continue.  A record-level failure (unreadable inode) is
	 * handled earlier and stays a mount-time limitation.
	 */
	...
	if (!(flags & (NTFS_MNT_RDONLY | NTFS_MNT_FORENSIC))) {
		...
		/*
		 * A dirty or Windows-cached $LogFile (a version 2.0 restart
		 * page, left by fast startup or an abrupt power cut) makes a
		 * filesystem driver refuse the volume, but fsck exists to
		 * repair exactly that state: it resets the journal itself,
		 * gated by its own repair policy.  Rejecting the mount here
		 * would make every repair mode fail on the volumes that need
		 * repair most, so leave the decision to fsck.
		 */
		if (!(flags & NTFS_MNT_FSCK) &&
