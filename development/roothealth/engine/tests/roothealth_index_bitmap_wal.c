...
static int finalize(struct rh_writer *writer, size_t first, size_t count,
		uint32_t evidence_version, uint64_t generation,
		const unsigned char before[32],
...
int main(int argc, char **argv)
{
	...
	struct rh_index_bitmap_census index_before = {0}, index_after = {0};
	...
	if (rh_cluster_bitmap_census_run(overlay.volume, &writer, 1,
			...
			rh_index_bitmap_census_run_from_raw(overlay.volume, &writer,
				&raw_before, 1, &index_before) || index_before.clean ||
			index_before.change_count != 1 ||
			rh_index_bitmap_stage(&overlay, &index_before, &first) || first != 3 ||
	...
	if (rh_cluster_bitmap_census_run(overlay.volume, &writer, 2,
			...
			rh_index_bitmap_census_run_from_raw(overlay.volume, &writer,
				&raw_after, 2, &index_after) || !index_after.clean ||
			index_after.change_count ||
			finalize(&writer, 1, 2, 1, cluster_before.generation,
				cluster_before.census_hash, cluster_after.census_hash) ||
			finalize(&writer, 3, 1, RH_INDEX_BITMAP_EVIDENCE_VERSION,
				index_before.generation,
				index_before.census_hash, index_after.census_hash))
		goto out;
	rh_ntfs_overlay_unmount(&overlay);
	mounted = 0;
	stage = "commit";
	...
	if (rh_wal_committed_accept(&wal, RH_WAL_TX_METADATA_REPAIR) ||
		...
		goto out;
	printf("index-bitmap-wal actions=24,24,13 entries=3 recovered=1 "
		"source_writes=%zu\n", writer.write_boundaries);
	result = 0;
out:
	if (result)
		fprintf(stderr, "index-bitmap-wal failed stage=%s errno=%d (%s)\n",
			stage, errno, strerror(errno));
	if (mounted)
		rh_ntfs_overlay_unmount(&overlay);
	rh_wal_uninstall_backend(&wal);
	rh_index_bitmap_census_destroy(&index_after);
	rh_index_bitmap_census_destroy(&index_before);
