...
def killselected(force=False):
    ...
    setstatus(f'{wording} {entry.get("name", "operation")}  {count} operation{"s" if count != 1 else ""}')
    queuesample(opsrequest({'op': 'LIST', 'resources': True, 'completed': True}))
    return True
...
def buildconfirm(commands):
    ...
    detail = f'this will signal the operation and {children} child operation{"s" if children != 1 else ""}.'
