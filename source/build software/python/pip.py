"""Native T1OS package manager for the system Python.
...
from operations.operations import architect_capability_check
...
def serve():
    ...
    try:
        ...
        os.chmod(path, 0o660)
        listener.listen(16)
        print('The T1OS system Python package service is ready.', flush=True)
        while True:
...
def main(argv=None):
    ...
    if values == ['diagnostic']:
        return diagnostic()
    if values:
        print('Use python.py with no arguments, or python.py diagnostic.', file=sys.stderr)
        return 2
    try:
        serve()
        return 0
